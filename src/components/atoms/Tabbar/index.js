...
export default class Tabbar extends Component {
	...
	render() {
		...
		_getIconStyle = (tab) => {
			if (this.props.activePage === tab.page) return { color: this.props.selectedIconColor };
					...
					return (
						<TouchableOpacity key={index} style={styles.iconView}>
							...
							>
								...
								<Text style={_getLabelStyle(tab)}>{tab.iconText}</Text>
							</Ripple>
						</TouchableOpacity>
					);
				});
		...
		return <View style={[ styles.tabBar, _getTabbarStyle() ]}>{_renderTabs()}</View>;
	}
}
